...
// wire	
  ...
  ushifter	ushifter_inst (
	...
	assign addr_o = (addr_sel == 1'b1)? a: p;
	assign s      = s_stack[sp];
	assign r      = r_stack[rp];
  ...
  always_comb 
	...
	begin
		if (clr) begin
			...
			if (pload     == 1'b1) begin p <= p_in ;end 
			if (aload     == 1'b1) begin a <= a_in;end 
			if (codeload  == 1'b1) begin code <= code_in ;end 
			if (phaseload == 1'b1) begin phase <= phase_in ;end 
			if (addrload  == 1'b1) begin addr_sel <= addr_in ;end 
			if (dataload  == 1'b1) begin data_sel <= data_in ;end 
			if (tload == 1'b1) begin t <= t_in;end 
			if (sload == 1'b1) begin s_stack[sp] <= t;end 
			if (spush == 1'b1) begin
				s_stack[sp1] <= t;
				sp <= sp + 1; sp1 <= sp1 + 1;end 
			if (spopp == 1'b1) begin
				sp <= sp - 1; sp1 <= sp1 - 1;end 
			if (rload == 1'b1) begin r_stack[rp] <= r_in;end 
			if (rpush == 1'b1) begin
				r_stack[rp1] <= r_in;
				rp <= rp + 1; rp1 <= rp1 + 1;end 
			if (rpopp == 1'b1) begin 
				rp <= rp - 1; rp1 <= rp1 - 1;end 
			if (inload == 1'b1) begin inptr <= inptr + 1 ;end 
			if (outload == 1'b1) begin outptr <= outptr + 1 ;end 
		end 
	end
	endmodule
